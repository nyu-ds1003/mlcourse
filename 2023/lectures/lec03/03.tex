...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Smaller $\sF$: less likely to overfit but can be farther from the Bayes prediction function
    \end{itemize}
    \pause
...
\item $\cf_{d}=\left\{ \mbox{linear functions using fewer than $d$ features}\right\} $
    ...
    \begin{itemize}
        \item Choose the subset of features that is best according to the score (e.g. validation error)
            ...
            \end{itemize}
        \item \al{No efficient search algorithm}; iterating over all subsets becomes very expensive with a large number of features 
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}[<+->]
        ...
        \item General approach to feature selection:
            ...
            \end{itemize}
        \item The search problem over $2^k$ feature subsets is hard
        \item Forward selection is often used in practice, but isn't guaranteed to find the best solution
        \item Forward and backward selection do not in general result in the same subset
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}[<+->]
        ...
        \item Bayesian intuition: pull the regression weights towards a prior centered at 0
    \end{itemize}
\end{frame}
...
\item Prediction function may generalize better
