...
f(x) = w^T\phi(x) .
\end{align}
\item This model is not linear in the inputs $x$
\item For example, we can use a feature map that define a kernel, \eg polynomial in $x$
\end{itemize}
...
\item Final \textit{linear} predictor uses the \textbf{intermediate features} computed by $h_i$'s:
\[
...
\item This doesn't explain why practical neural networks work, or tell us how to build them
\end{itemize}
...
\item Last layer typically gives us a score. How do we perform classification?
\end{itemize}
\end{frame}
%
