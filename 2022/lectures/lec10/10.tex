...
\item[Solution] \emph{Greedy} algorithm: starting from the root, and repeating until a stopping criterion is reached (\eg max depth),
\begin{itemize}[<+->]
    \item Find the non-terminal node that results in the ``best'' split (We only split regions defined by previous non-terminal nodes)
\item A greedy algorithm is the one that make the best \textbf{local} decisions, without lookahead to evaluate their downstream consequences
    \item This procedure will very likely not result in the globally optimal tree
\end{itemize}
\item[Prediction] Our prediction is the mean value of a node: $\hat y_{R_m} = \text{mean}(y_i \mid x_i \in R_m)$
\end{description}
...
\item Let $x_{j(1)},\ldots,x_{j(n)}$ be the sorted values of the $j$-th
feature.
\item We only need to consider split points between two adjacent values.
\item Any split point in the interval $(x_{j(r)}, x_{(j(r+1)})$ will result in the same loss 
...
\]
\end{description}
\item The Gini index and entropy are numerically similar and work better in practice than the misclassification error.
...
\begin{itemize}
\item \emph{Non-linear} classifiers that recursively partition the input space.
\item Non-metric: they make no use of geometry, \ie they do not rely on inner products or distances.
\item Non-parametric: they make no assumptions about the data distribution.
\end{itemize}
\end{simpleblock}
...
\begin{itemize}
\item Simple to understand: interpretable.
\end{itemize}
\end{simpleblock}
}
\onslide<.->{
...
\item They have high variance and tend to \al{overfit} (sensitive to small changes in the training data). The ensemble techniques we discuss next can mitigate this issues.
\end{itemize}
\end{simpleblock}
}
...
\begin{definition}
A \textbf{bootstrap method } simulate having $B$
...
\item Increasing the number of trees does not lead to overfitting

\item Is there a downside, compared to a single decision tree?

\item The bagged predictor is much less interpretable
...
\item For large $n$, the covariance term dominates -- limits benefit of averaging.
\end{itemize}
\end{frame}
...
\end{frame}
%
\begin{frame}{Random Forest: Effect of $m$} 
\begin{figure}
...
\item Bagging works better when we combine a diverse set of
...
\item Examples with larger weights have greater influence on the loss.
...
\begin{itemize}
\item Find base classifier $G_{m}(x)$ that on weighted
training data (this classifier may not fit the data well)

\item Increase weight on the points that $G_{m}(x)$ misclassifies (key idea of boosting)
...
\item smaller when $G_{m}$ fits its weighted $\cd$ less well
\end{itemize}
\end{itemize}
...
\begin{itemize}
    \item We train $G_{m}$ to minimize weighted error, and it achieves $\text{err}_{m}$.
...
\item If $G_m$ is good, then $\alpha_m$ will be large, so if $x_i$ is misclassified,
    its weight will increase to a greater extent.
...
\par\end{centering}
\caption{{\footnotesize{}Size represents weight. Shade of grey represents
score for red class.}}
\end{figure}
...
\item Very powerful off-the-self classifier
\item Next week
\begin{itemize}
